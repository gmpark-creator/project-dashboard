...
const PROJECTS = [
  ...
  },
  {
    ...
    preview: { type:'embed', height:600, items:[
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/us-kr-premarket/kr-sys-manual/', label:'⑫ KR SYS 지표 판독 매뉴얼 (2026-08-31 편입) — 박사가 TradingView에 올린 자작 지표 KR SYS 1(메인 차트)·KR SYS 2(하단 패널)를 색깔별로 읽는 법과, 골든크로스 자리에 피보나치 되돌림을 대는 순서를 담은 문서. 박사가 바탕화면 파일(KR_SYS_reference.html)로 전달 → 원문 무수정 편입. ★1부 메인 차트: 이평선 5(청록)·20(빨강, 볼린저 중심선과 동일)·60(주황, 수급선)·120(보라, 굵게)·200(파랑, 굵게) — 120·200을 굵게 그린 이유는 조건①의 판정선이기 때문. 더블 볼린저 ±1σ/±2σ 4선, 1σ~2σ 사이 매수·매도존 음영, 조건① 미충족 구간 전체 붉은 배경(떨어지는 칼날 차단), ENTRY 초록삼각형(6/6 충족)·주황동그라미(5/6). ★2부 하단 패널: RSI(보라)·MACD(파랑)·시그널(주황), 두 선 사이 음영이 히스토그램 역할이며 폭이 좁아지는 초록은 위험 신호. 설계 핵심은 RSI 50선과 MACD 0선을 같은 위치에 겹쳐 가로선 하나로 두 지표를 동시 판정하게 만든 것. 하늘색 다이아몬드 = MACD 0선 상향돌파(0선 위 골든크로스가 아래보다 신뢰도 높음). ★3부 피보나치 6단계: 골든크로스 날짜 확정 → 직전 하락 파동 저점 → 그 하락이 시작된 고점 → 고점에서 저점으로 드래그 → 38.2~61.8%가 진입 구간 → 0%선 이탈 손절·78.6% 1차·100% 2차 목표. 사례는 ADI 2025-11-24(⑪ 스크리너가 잡은 나스닥 신호와 동일 건). ★Claude 실측 대조(2026-08-31): ENTRY 종가 239.40과 되돌림 55.8%는 실측과 정확히 일치. 다만 앵커 3개가 실제 봉과 불일치 — 문서의 고점 250.00@2025-10-08은 그날 실제 고가가 239.36이고(250선 고가는 9/11·9/17·9/18·9/23), 저점 226.00@2025-11-21은 그날 실제 저가가 225.00이며 그 구간 최저도 아님(11/07 223.47, 11/20 224.25). 실측 앵커(고 252.00@9/18 - 저 223.47@11/07)로 재계산하면 되돌림이 55.8%로 문서와 우연히 동일해 결론은 유지되나, 진입 구간은 234.37~241.10, 손절 0선은 223.47(문서 226.00 대비 2.53달러 아래), 1차 목표 245.89로 달라진다. 또한 문서 2·3단계를 실제 봉에 문자 그대로 적용하면 직전 파동이 고 242.00@11/12 - 저 224.25@11/20이 되어 되돌림 85.4%(구간 밖)로 진입 불가 판정이 나오므로, 어느 파동을 잡느냐가 결과를 가른다. 원문이 스스로 「차트 이미지에서 눈으로 읽은 근사치이니 TradingView에서 정확한 고가·저가로 다시 계산하라」고 명시하고 있어 원문 무수정 편입했고, 정정 여부는 박사 판단 대기. ★문서가 스스로 경고하는 최대 실수: 저점 미확정 상태에서 피보나치를 긋는 것 — 신저가마다 0%선이 내려가 어제의 38.2%가 오늘 23.6%가 된다. ⑪에서 실패한 MCHP·ARM·MPWR이 이 상태였다고 지목. 밝은 지면(paper) 디자인 원문 유지. 연구·교육용·투자자문 아님.' },
