...
const PROJECTS = [
  ...
  },
  {
    ...
    subtitle: '증권 — 미국·한국 반도체 단타(당일매매) 의사결정 지원 터미널 · 실시간·기록 3종(①SOXL 실시간 ②Premarket Signal ③6월 시세·이벤트) + 신규 트레이딩 도구·데이터 5종(④단타 데스크 ⑤시그널 엔진 ⑥0%선 전환 리플레이어 ⑦데이터·지식 베이스 ⑧단타 생존 가이드 5원칙) + 디렉터 코멘트(실전 트레이딩 로그) + 암호화폐 크로스 워치(BTC 골드/데드크로스) + ⑨차트 교본(종목 중립 — 단타·스윙 정의·캔들 도감·지표 계층·용어 100·구조적 마찰 관측기) + ⑩SKHY 관측 대시보드(SK하이닉스 나스닥 ADR — 발행구조 원문 확정·두 시점 병기 가격괴리·SEC 공시, 경로 판별은 데이터 미확보로 판정 불가) + ⑪눌림목 반등 스크리너(6조건 동시충족 조건식을 나스닥100·코스피200 303종목에 전수 적용 — 신호 0개 종목·손실 사례 포함 전량 기록, 생존편향 미보정 명시) + ⑫KR SYS 전체 판독표 v2(박사 자작 TradingView 지표 KR SYS 1·2의 모든 선·음영·마커·표 색깔별 판독 — 2026-09-01 v2 전면 개정으로 거래량 막대 4색·WPR(21)·다이버전스 4종·화면 표 13줄/6줄·실전 판독 순서 8단계·세 종목 비교 예시 추가, ⑪과 동일 6조건의 육안 판정편) — Claude·Codex 변증법 구축',
    icon: 'trending-up',
    ...
    preview: { type:'embed', height:600, items:[
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/us-kr-premarket/kr-sys-manual/', label:'⑫ KR SYS 전체 판독표 v2 (2026-08-31 편입 → 2026-09-01 v2 전면 개정) — 박사가 TradingView에 올린 자작 지표 KR SYS 1(메인 차트)·KR SYS 2(하단 패널)의 모든 선·음영·마커·표를 색깔별로 읽는 문서. 박사가 바탕화면 파일(KR_SYS_reference_v2.html)로 전달 → 원문 무수정 교체(22,718바이트 바이트 동일 확인). ★1부 메인 차트: 이평선 5(청록)·20(빨강, 볼린저 중심선과 동일)·60(주황, 수급선)·120(보라, 굵게)·200(파랑, 굵게) — 120·200을 굵게 그린 이유는 조건①의 판정선이기 때문. 더블 볼린저 ±1σ/±2σ 4선과 1σ~2σ 매수·매도존 음영. 피보나치는 자동 작도로 바뀌어 0%/100%(피벗 자동 탐지) 굵은 선, 38.2~61.8% 노란 진입 구간, 23.6·50·78.6% 보조선. 거래량 막대 4색(주황=20일 평균 1.5배 이상으로 조건⑥ 충족 / 파랑=1.0~1.5배 / 연청록=평균 이하 양봉 / 연빨강=평균 이하 음봉)에 20일 평균 노란 선, 막대 높이는 최근 200봉 최대치 대비 상대값이라 절대 수치는 표의 배수로 확인. 조건① 미충족 구간 전체 붉은 배경(떨어지는 칼날 차단), 시장 필터 주황 배경은 검증 결과 유효하지 않아 기본 꺼짐. 우측 상단 화면 표 13줄 판독법(추세 3중 / 하단 이탈 10봉 / 안쪽 밴드 회복 / RSI 45-60 + 방향 / MACD GC 경과 봉수와 0선 위아래 / 거래량 배수 / 시장 참고 / 충족 n·6 / 되돌림 % / GC 시점 되돌림 / 저점 상태 / 손절·리스크). ★2부 하단 패널: RSI(보라, 원본 0~100)·MACD(파랑, 0~100 정규화라 화면 50 = 실제 0)·Signal(주황)에 WPR(21, 하늘색, -100~0에 +100을 더해 0~100으로 이동)이 추가됐다. 두 선 사이 음영이 히스토그램 역할이며 폭이 좁아지면 꺾이는 중. 설계 핵심은 RSI 50선과 MACD 0선을 같은 위치에 겹쳐 가로선 하나로 두 지표를 동시 판정하게 만든 것. 마커 4종(▲ 골든크로스 / ▼ 데드크로스 / ◆ 하늘 = MACD 0선 상향 돌파로 가장 중요 / ◆ 자홍 = 0선 하향 이탈). 다이버전스 4종은 선 모양으로 성격이 갈린다 — 실선 = 추세 전환(R Bull·R Bear), 점선 = 추세 지속(H Bull·H Bear)이고 R은 RSI·M은 MACD 기준이라 같은 자리에 R과 M이 동시에 뜨면 신뢰도가 높다. 원문이 스스로 경고: 상승 다이버전스는 가격이 새 저점을 만들어야 성립하므로 반등 중에는 나오지 않고, 하락 추세에서는 반복해 뜨고도 계속 떨어질 수 있어 단독 진입 근거로 쓰면 안 된다. 좌측 하단 표 6줄(MACD·Signal·Hist·RSI·WPR·상태)도 정리. ★3부 실전 판독 순서 8단계: 배경이 붉은가(붉으면 즉시 다음 종목) → 충족 n/6 → 저점 상태가 초록인가 → 되돌림 38.2~61.8 안인가 → MACD 부호 → Hist 방향 → 거래량 막대 주황 여부 → 손절가·손실률 감당 가능 여부. 1~2번만 봐도 90%가 걸러진다고 명시. 끝에 세 종목 비교 예시(문서 표기 2026-09-02 기준) — AAPL 추세 3중 O·O·O에 되돌림 37.0%로 3/6 진입 구간 문턱, MSTR 되돌림 298.4%로 반등 후반부, APP MACD -27.50에 되돌림 10.4%로 바닥 미확인. 같은 3/6·2/6이어도 어느 조건이 X인지에 따라 의미가 다르다는 것이 결론. ★문서가 스스로 경고하는 최대 실수: 저점 미확정 상태에서 되돌림 숫자를 믿는 것 — 신저가마다 0%선이 내려가 기준이 흔들린다. ⑪에서 실패한 MCHP·ARM·MPWR이 이 상태였다고 지목(v1부터 유지). ★v1 대비 변경: 수기 작도 6단계와 ADI 2025-11-24 워크드 예시가 v2에서 삭제되고 자동 피보나치 + 화면 표 판독으로 대체됐다. 2026-08-31 Claude ADI 실측 대조 기록(앵커 3건 불일치·되돌림 55.8% 일치)은 아래 이력·타임라인에 그대로 보존. ★확인 필요: 세 종목 비교 예시의 기준일이 2026-09-02로 적혀 있는데 교체 시점은 2026-09-01이라 하루 앞선 날짜다. 원문 무수정 원칙에 따라 손대지 않았고 정정 여부는 박사 판단 대기. 밝은 지면(paper) 디자인 원문 유지. 연구·교육용·투자자문 아님.' },
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/us-kr-premarket/rebound-screener/', label:'⑪ 눌림목 반등 스크리너 (2026-08-31 신설) — 박사가 지정한 6조건 동시충족 조건식(①추세: 종가>200일선 + 200일선이 20일 전보다 높음 + 종가>120일선 ②최근 10봉 내 볼린저 −2σ 이탈 이력 ③당일 종가>−1σ ④RSI(14) 45~60 이면서 전일보다 상승 ⑤최근 5봉 내 MACD 골든크로스 ⑥거래량>20일평균×1.5)을 2020-01-01~2026-08-31 구간에 적용. ★핵심 규율은 부합 사례만 추려오지 않는 것 — 나스닥100 102종목 + 코스피200 201종목 = 303종목을 전수 순회하고 신호 0개 종목도 0으로 기록했다. 결과 352건 / 종목당 평균 1.18건 / 그중 109종목(36.5%)은 0건. ★시세 출처는 이중화·교차검증 — 미국은 yfinance(Yahoo, auto_adjust=False의 Close = 분할 반영·배당 미반영), 한국은 pykrx(KRX 정보데이터시스템, adjusted=True). 삼성전자 2026-08-28 종가가 pykrx 257,000 = yfinance 005930.KS 257000 으로 일치 확인. 편입리스트는 Wikipedia List of NASDAQ-100 companies + 네이버 금융 KOSPI200 편입종목 21p(pykrx 지수 구성종목 API는 KRX 로그인 요구로 사용 불가). ★데이터 결함 1건 발견·수정 — pykrx가 거래정지 구간을 OHLC=0·거래량=0 가짜 봉으로 채워 반환하는 탓에 SK텔레콤(SK스퀘어 인적분할 정지 2021-10-26~11-26) 최대낙폭이 −100%로 계산됐다. 27종목 278행 제거 후 전량 재계산해 −7.41%로 정정. ★관측 결과(판단·추천 아님): 20일 승률 51.7%(평균 +1.99%·중앙값 +0.37%) / 60일 승률 56.8%(평균 +9.70%·중앙값 +2.75%) / −8% 손절 터치 59.5%. 평균과 중앙값이 3.5배 벌어져 있고 상위 5건을 빼면 60일 평균이 7.10%로 내려간다. 신호는 2021년에 99건으로 몰려 있고 하락장인 2022년은 16건 — 조건①이 하락장을 걸러내기 때문. ★한계 전면 고지 — 편입리스트가 2026-08-31 현재 구성이라 그 사이 지수 퇴출·상장폐지된 종목이 통째로 빠진 생존편향 상태이며(시점별 편입이력은 유료 데이터라 확보 불가), 거래비용·세금·슬리피지 미반영, 같은 국면의 연속 신호를 중복 계상, 2026년 6월 이후 신호는 T+60 미도래로 추정 없이 공란 처리. 재현용 원시 CSV(A_signals 352행·B_counts 303행)와 실행 코드 전체를 data/ 에 동봉. 연구·교육용·투자자문 아님.' },
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/us-kr-premarket/skhy-tracker/', label:'⑩ SKHY 관측 대시보드 (2026-08-13 신설) — SK하이닉스 나스닥 ADR(SKHY, 2026-07-10 상장) 관측 전용. 박사가 별도 스펙 문서(웹 Claude 작성)를 주고 구현 지시 → Claude Phase 1 실측 → Codex(gpt-5.6-sol) 적대검토 `PHASE1 DISPUTED` → 정정 후 축소 구현. ★원 스펙의 목적(경로 A=ADR 유통시장 흡수 vs 경로 B=신규 ADS 발행 판별)은 데이터 미확보로 달성 불가 — data.krx.co.kr 통계 API가 인증 뒤로 이동(HTTP 400·본문 "LOGOUT", 쿠키·Referer·AJAX 헤더 선행에도 동일)했고 KRX OpenAPI 목록에 외국인 보유수량·종목별 투자자 순매수가 없어 계정 발급만으로 해결 안 됨. 추정·대체소스·보간으로 채우지 않고 「판정 불가」를 전면에 표시. ★SEC 원문 확정 사실: 1 ADS = 보통주 1/10주(F-6 원문 인용) / IPO 본주 17,790,000주 = ADS 177,900,000주(424B4) / 공모 전 발행총수 712,702,365주(424B4 직접 인용) → 공모 후 발행총수 730,492,365주(파생) vs outstanding 728,865,500주(자기주식 제외) 개념 구분 / issuer CIK 0002120882이나 F-6 archive registrant CIK는 1472033로 달라 문서 URL 조립 시 깨짐. ★지분율 분모는 고정 상수가 아니라 날짜별 상장주식수 — 공개표에서 7/28 보유373,374,636+매수가능339,327,729=712,702,365 → 7/29 371,422,583+359,069,782=730,492,365로 정확히 신주 17,790,000주만큼 전환됨을 산술 확인. 반올림 지분율 역산 금지(분모 선택만으로 최대 1,515만주, 반올림만으로 ±36,525주=ADS ±365,250주 오차). ★ADR 가격 괴리는 단일 수치 금지, 두 시점 정의 병기 — ①동일 날짜 라벨 15.33~60.11%(평균 32.76%, n=18) ②한국 종가 시점 기지 19.28~43.57%(평균 33.98%, n=17). 원인 미확정(비동시 비교·예탁 제한·시장분절·대차부족·편입기대 선반영·ADS outstanding 오인 6후보 병기). ★거래 규모는 종가×거래량 proxy임을 필드명·화면에 명시. ★사건 타임라인은 확정/관측/예정 3구분, 지수 편입은 미확정이며 $20~40억은 시나리오로만 표기. ★수집 receipt(URL·UTC 수집시각·HTTP status·바이트·SHA-256) 노출, 원본 응답은 .gitignore로 공개 저장소 커밋 차단, TTL 30시간 초과 시 stale 배너. ★검증 게이트 verify.mjs — 확정 수치 원문 대조·결측 0대체 금지·두 정의 병기·proxy 표기·차단 소스 노출·금지 표현(경로 우세/파이프 개폐/신뢰도 라벨/편입 확정) 부정문맥 밖 사용 차단, 음성 테스트 통과. 연구·교육용·투자자문 아님.' },
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/us-kr-premarket/chart-playbook/', label:'⑨ 차트 교본 (2026-08-13 신설) — 종목 중립 차트 읽기 자료. 9섹션: ①스타일 정의(스윙=2~20거래일: 숏 2~5·코어 6~10·롱 11~20 / 단타=스캘핑+인트라데이, 정규장 종가 미통과 / 오버나이트 초단기는 별도 브리지 + Fidelity·Schwab 등 출처별 정의 병기로 "업계 통일 정의 없음" 명시) ②데이터 계약(D0 게이트 7종: 신선도·수정주가·기업행사·거래정지·세션은 전체 차단, 창 미달·호가 부재는 축 단위 차단) ③타임프레임 3역할(시장구조/셋업/체결 — 배율 고정 규칙은 자기모순으로 폐기) ④지표 계층(최상위=가격 OHLC+거래량 원자료, 하나만 덧붙이면 ATR(사용 봉·세션 병기) — 방향 예측이 아니라 고정 손절폭 착시 방지 목적 + 이평선 SMA/EMA/WMA/VWMA 종류·기간 관례·색: TradingView 공식 기본은 길이 9 단일이며 9·21·50·200은 사용자 관례, 키움 이평 기본 색·기간은 공개 문서 미확인으로 단정하지 않음) ⑤캔들·꼬리 도감 18종(한계 7종을 도감 상단에 고정 — 캔들은 신호·확률이 아님, 연구 결과 엇갈림 SAGE·Mathematics 인용 / 한·미 색 반전 경고: 같은 빨강이 한국=상승 미국=하락 / 색 외 ▲▼·실선·점선 이중 부호화) ⑥셋업 복기 4종(이동평균 되돌림·박스권 이탈·이탈 후 재확인·개장 레인지 — 관측→확인→무효화→실패각주 4블록, 운영 숫자 0, 무효화에 "자동 손절가 아님" 고정. 성공/실패 짝은 같은 시드로 관측 컷오프 이전 봉이 완전 동일) ⑦종목-스타일 구조적 마찰 관측기 9축(A1 유동성·A2 스프레드·A3 ATR%·A4 거래량 일관성·A5 갭·A6 구조 명료성·A7 상대강도·A8 상품구조 하드플래그·A9 시장깊이/가격충격 + D0 게이트. 출력은 관측값과 산출 완료/부분 산출/산출 불가까지이며 임계값이 없어 낮음·높음 범주 판정과 종목 추천을 하지 않음. 계획 손실거리는 사용자 직접 입력, ATR 자동 대입 없음) ⑧용어집 100개(눌림목 심화 포함 — 룩어헤드 경고·피보나치 38.2~61.8%는 보편 경계 아님) + 관행 주장 레지스트리(출처 미특정 수치는 라벨 없이 삭제, 현재 0건) ⑨방법론·출처 20건·면책. 캔들은 외부 라이브러리 없이 인라인 SVG 직접 렌더, 합성 OHLC는 시드 고정 결정론 생성(재생성 바이트 동일), 사전조건은 RFC 8785 JCS→SHA-256 해시로 검증기가 재계산 대조. 한국식(적/청)·TradingView식(청록/적) 프리셋 토글. 연구·교육용·투자자문 아님.' },
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/us-kr-premarket/', label:'Polaris 허브 — 상단에 BTC 실시간 시세 패널(해외 Coinbase·Kraken·Gemini USD현물 + Binance USDT + USD평균 / 국내 업비트·빗썸 KRW, 1초 폴링·상승빨강 하락파랑) + 4섹션(디렉터 코멘트 + 암호화폐 크로스 워치 + 실시간·기록 3종 + 트레이딩 도구·데이터 5종) 카테고리 선택창 (2026-06-24 실시간 티커+BTC 워치 추가)' },
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/us-kr-premarket/btc-crosswatch/', label:'BTC 골드/데드크로스 워치 — 브라우저에서 Binance 일봉 직접 받아 SMA 실시간 계산. 기준 토글 5/20(단타·기본)·50/200(클래식) + 인라인 그래프(종가+단기선+장기선+교차 마커, canvas)·판정 배너·핵심수치·두 기준 비교·교차 이력·TradingView(MA5·20·50·200). 비트코인 일봉 50/200일 SMA 교차 상태 요약(레거시). Codex가 Binance BTCUSDT 1D klines + CoinGecko BTC/USD daily 기준 SMA 직접 계산. 판정: 현재 골든크로스 아님(50일선이 200일선보다 약 $5.2k 아래) — 2025-05-22 골드크로스 → 2025-11-16 데드크로스 → 이후 50<200 유지. 당일(2026-06-24 진행중 일봉) 장중 +0.4% 반등 반영. 핵심수치 패널·교차 이력 타임라인·영문 Market Summary·TradingView 라이브 차트(외부)·미확정 캔들 주의. 연구·교육용·투자자문 아님. (2026-06-24 신설)' },
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/us-kr-premarket/broadcom-oracle-earnings/', label:'브로드컴·오라클 실적 후 주가 + SOXL/SOXS — 2026년 AVGO(Q2 FY26, 6/3 장후)·ORCL(Q4 FY26, 6/10 장후) 실적 발표 당일·다음날·1주일 본주 종가 + 반도체 3배 ETF(SOXL 불·SOXS 베어) 반응. 둘 다 호실적인데 급락: AVGO 6/4 -12.6%(AI 연간목표 미상향·Q3 AI 가이던스 $16B<$17.2B), ORCL 6/11 -8.5%(FY27 $400억 조달·FCF -$237억·차입 우려). AVGO 약한 AI 가이던스가 6/5 반도체 섹터 급락(보도 기준 반도체지수 -10%대·$1T+ 증발)의 방아쇠 → SOXL 6/5 -30.5%·SOXS +31.5%. ORCL은 반도체 아니라 반응일 SOXL +24% 디커플링. 일별표(6/1~6/18)·레버리지 decay 경고(SOXL≈제자리 vs SOXS -26.9%). 데이터=Yahoo+stockanalysis 교차검증, Codex R1·R2 검수. 연구·교육용·투자자문 아님. (2026-06-24 신설) ' },
      ...
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/us-kr-premarket/survival-guide/', label:'⑧ 단타 생존 가이드 5원칙 — 토스증권 화면 기준 5원칙 인터랙티브 인포그래픽 (2026-06-16)' }
    ]},
    ...
    issues: [
      { type:'완료', title:'KR SYS 판독 매뉴얼 v2 전면 교체 — 거래량·WPR·다이버전스·화면 표 판독 편입 (2026-09-01)', desc:'박사가 바탕화면 파일 KR_SYS_reference_v2.html 로 ⑫ 페이지를 교체하라 지시 → previews/us-kr-premarket/kr-sys-manual/index.html 을 원문 무수정 덮어쓰기(21,897바이트 v1 → 22,718바이트 v2, md5 바이트 동일 확인). v1은 이평선·볼린저·하단 패널 3선 + 수기 피보나치 작도 6단계 구성이었는데, v2는 화면에 실제로 그려지는 요소를 전수 정리하는 쪽으로 성격이 바뀌었다. 추가된 것: 거래량 막대 4색 체계(주황=20일 평균 1.5배 이상 = 조건⑥ 충족, 파랑=1.0~1.5배, 연청록·연빨강=평균 이하 양봉·음봉)와 막대 높이가 최근 200봉 최대 대비 상대값이라는 주의, 하단 패널 4번째 선 WPR(21)(원래 -100~0을 +100 이동해 0~100으로 표시, RSI보다 먼저 반응하는 조기 경보이나 단독 판단 금지), 다이버전스 4종(실선=추세 전환 R Bull·R Bear / 점선=추세 지속 H Bull·H Bear, R=RSI·M=MACD 동시 출현 시 신뢰도↑, 상승 다이버전스는 새 저점이 있어야 성립하므로 반등 중에는 안 나온다는 경고 포함), 우측 상단 화면 표 13줄·좌측 하단 표 6줄의 줄별 판독법, 실전 판독 순서 8단계, 세 종목 비교 예시(AAPL 3/6 진입 구간 문턱 / MSTR 되돌림 298.4% 반등 후반부 / APP MACD -27.50 바닥 미확인). 피보나치는 수기 드래그 6단계에서 피벗 자동 탐지(0%·100% 굵은 선, 38.2~61.8% 노란 진입 구간)로 바뀌었고 ADI 2025-11-24 워크드 예시는 v2에서 삭제됐다 — 2026-08-31 실측 대조 결과는 아래 항목에 그대로 보존한다. ★확인 필요 1건: 세 종목 비교 예시의 기준일이 2026-09-02로 적혀 있으나 교체 시점은 2026-09-01이라 하루 앞선 날짜이며, 해당 수치는 Claude가 실측 대조하지 않았다(박사 제공 원문 그대로). 원문 무수정 원칙에 따라 손대지 않았고 정정 여부는 박사 판단 대기. 대시보드 카드 문구도 v2 구성(1부 메인 차트+피보나치+거래량+표 13줄 / 2부 패널 4선+마커+다이버전스+표 6줄 / 3부 판독 순서 8단계)에 맞춰 갱신. 연구·교육용·투자자문 아님.' },
      { type:'완료', title:'KR SYS 지표 판독 매뉴얼 편입 + ADI 피보나치 수치 실측 대조 (2026-08-31)', desc:'박사가 바탕화면 파일 KR_SYS_reference.html 을 대시보드에 추가하라 지시 → previews/us-kr-premarket/kr-sys-manual/ 로 원문 무수정 편입(21,897바이트 바이트 동일 확인, 밝은 paper 디자인 원문 유지). 내용은 박사 자작 TradingView 지표 KR SYS 1(메인 차트)·KR SYS 2(하단 패널)의 선 판독법 + 골든크로스 자리 피보나치 되돌림 작도 6단계이며, ⑪ 스크리너와 동일한 6조건을 화면에서 육안 판정하는 쪽의 문서다. 설계상 눈에 띄는 점: 120·200일선을 굵게 그린 이유가 조건①의 판정선이라서이고, 하단 패널은 RSI 50선과 MACD 0선을 같은 좌표에 겹쳐 가로선 하나로 두 지표를 동시 판정하게 만들었다. ★Claude 실측 대조 수행(yfinance ADI 일봉) — 일치: ENTRY 종가 239.40, 되돌림 55.8%. 불일치 3건: 고점 250.00@2025-10-08은 그날 실제 고가가 239.36(250선 고가는 9/11 250.00·9/17 250.20·9/18 252.00·9/23 250.36) / 저점 226.00@2025-11-21은 그날 실제 저가가 225.00 / 그 저가는 구간 최저도 아님(11/07 223.47, 11/20 224.25가 더 낮음). 실측 앵커(252.00@9/18 - 223.47@11/07)로 재계산 시 되돌림이 55.8%로 우연히 동일해 진입 구간 안이라는 결론은 유지되지만, 파생 가격은 달라진다 — 진입 구간 234.37~241.10(문서 235.17~240.83), 손절 0선 223.47(문서 226.00), 1차 목표 245.89(문서 244.86). ★더 중요한 발견: 문서 2·3단계(골든크로스 직전 저점 → 그 하락이 시작된 고점)를 실제 봉에 문자 그대로 적용하면 직전 파동이 242.00@11/12 - 224.25@11/20이 되어 되돌림 85.4%, 즉 38.2~61.8% 구간 밖이라 진입 불가 판정이 나온다. 실제 ADI는 10/10·11/07·11/20에 저점이 세 번 찍힌 박스권이라 어느 파동을 잡느냐가 결과를 가르며, 문서의 절차 서술만으로는 그 선택이 일의적으로 정해지지 않는다. ★원문이 스스로 「차트 이미지에서 눈으로 읽은 근사치이니 TradingView에서 정확한 고가·저가로 다시 계산하라」고 명시하고 있어 박사 문서를 임의 수정하지 않고 원문 그대로 편입했으며, 수치 정정과 파동 선택 규칙 명문화 여부는 박사 판단 대기. 연구·교육용·투자자문 아님.' },
      { type:'핵심', title:'눌림목 반등 스크리너 신설 — 6조건 조건식 303종목 전수 검증 (Claude, 2026-08-31)', desc:'박사가 조건식 6개(①추세 필터: 종가>200일 SMA + 200일선이 20일 전보다 높음 + 종가>120일 SMA ②최근 10봉 내 볼린저 −2σ 하향 이탈 이력 ③당일 종가>−1σ ④RSI(14) 45~60 이면서 전일보다 상승 ⑤최근 5봉 내 MACD(12,26,9) 골든크로스 ⑥거래량>20일 평균×1.5)와 절대규칙 4개(확인 불가 수치 날조 금지 / 부합 사례만 골라오지 말고 전 종목 순회하며 0개도 0으로 기록 / 하락 사례 빠짐없이 포함 / 출처 명시)를 직접 지정. ★유니버스는 박사 선택으로 둘 다 — 나스닥100 102 + 코스피200 201 = 303종목, 2020-01-01~2026-08-31, 20종목 16배치로 분할 실행. ★가능성 사전 검증: yfinance 1.5.2·pykrx 1.2.8·pandas 설치 및 네트워크 실측 확인 후 착수. pykrx의 지수 구성종목 API는 KRX_ID/KRX_PW 로그인을 요구해 사용 불가 → 코스피200 편입리스트는 네이버 금융 21페이지 파싱으로 대체(신규 분할지주사 코드에 알파벳이 섞여 isdigit 필터로 2종목 누락 → isalnum 으로 정정, 최종 201). ★엔진 신뢰성 확보: AAPL 2026-01-30 신호를 6조건 전부 원자료 숫자로 수동 대조(SMA200 236.65·SMA200[t-20] 231.75·−2σ 이탈일 2026-01-20 종가 246.70<249.88·RSI 48.44>전일 46.32·골든크로스 2026-01-28·거래량 92,443,400>78,001,298) 후 배치 실행. 볼린저 STD는 모표준편차(ddof=0), RSI는 Wilder 평활, 거래량 20MA는 당일 포함(HTS 기본). ★데이터 결함 1건 자체 적발·수정 — pykrx가 거래정지 구간을 open=high=low=0·volume=0·close=직전종가 인 가짜 봉으로 채워 반환한다. SK텔레콤(SK스퀘어 인적분할 정지 2021-10-26~11-26)에서 저가 0이 잡혀 최대낙폭 −100%·손절 터치라는 허위값이 나왔고, OHLC 0 이하 행 제거 필터를 넣어 27종목 278행을 걷어낸 뒤 전량 재계산해 −7.41%·미터치로 정정(신호 개수 자체엔 영향 없음). ★결과: 신호 352건 / 종목당 평균 1.18건 / 신호 0개 109종목(36.5%) / 데이터 워밍업 220봉 미달로 판정불가 4종목(HONA·SPCX·삼성에피스홀딩스·한화머시너리앤서비스홀딩스)은 조회 불가가 아니라 판정불가로 구분 기록. 20일 승률 51.7%(평균 +1.99·중앙값 +0.37) / 60일 승률 56.8%(평균 +9.70·중앙값 +2.75) / −8% 손절 터치 59.5%. ★거래량 20MA를 당일 제외 방식으로 바꾸면 352→372건(+5.7%)이라는 민감도도 함께 산출. ★한계 명시(박사 승인 하에 감수) — 편입리스트가 현재 구성이라 지수 퇴출·상폐 종목이 빠진 생존편향으로 승률이 과대추정되며, 시점별 편입이력은 유료 데이터라 보정 불가. 거래비용·세금·슬리피지 미반영, 연속 신호 중복 계상, T+60 미도래 구간은 추정 없이 공란. 결과 페이지 전면과 각 표 상단에 경고를 고정 표시. ★산출물: previews/us-kr-premarket/rebound-screener/ (정렬·필터 가능한 352행 신호표 + 303종목 개수표) + data/ 에 원시 CSV 2종·final.json·실행 코드 4종 동봉해 재현 가능. 연구·교육용·투자자문 아님.' },
      { type:'핵심', title:'차트 교본 신설 + Polaris Core v2 마이그레이션 — Claude·Codex 변증법 5라운드 (2026-08-13)', desc:'박사 지시 8항(①차트 지표 토론 3회 이상 ②단타/스윙 정의 ③보유기간별 차트 구성 ④분봉·일봉·캔들·꼬리와 진입 시점 ⑤트레이딩뷰풍 가시성 프론트엔드 ⑥눌림목 등 용어 상세 ⑦이평선 종류·색·캔들 색 ⑧Claude가 gpt-5.6-sol 호출해 협업). ★진행 중 박사 전제 정정 하달 — "SOXL로 체결하지 않는다. 새로운 종목을 발굴하는 것이다. 밸류에이션은 내가 찾고 차트는 너희가 준다. SOXL 하나에 매몰되지 마라." → 산출물을 종목 중립 프레임으로 전환하고 「차트 기반 종목 적합성 축」 신설, SOXL은 레버리지 예외 경고 사례로 강등. 오염된 전제로 실행 중이던 R2를 중단(TaskStop)하고 v2 재발행. ★변증법 5라운드(Codex EXEC gpt-5.6-sol read-only xhigh): R1 DESIGN BLOCK(T1~T8 전항목, 반박 14건) → R2 BLOCK(+GO 체크리스트 20항) → R3 BLOCK(5·13·15·16·20 미충족) → R4 BLOCK(13·15·20) → R5 IMPLEMENTATION GO(미해소 0). Codex가 잡아낸 것: 내 타임프레임 배율 규칙이 제시표 스스로를 위반(3분→1분=3배, 일봉→15분≈26배), 기존 core-data.js의 "단타=당일 청산"과 신규 정의 충돌, "1일 매매횟수 5~50회" 표기가 과매매를 정상화, 꼬리는 체결 경로일 뿐 매수·매도 원인을 증명하지 않음(저유동성·단일가·시간외 이상체결로도 길어짐), 미검증 수치를 라벨 붙여 남겨도 행동 앵커로 작동하므로 출처 미특정 시 삭제, 산술값 산출과 낮음/높음 범주 판정은 별개(임계값 없으면 "산출 완료"까지만), 1×ATR 자동 손실거리 대입은 그 자체가 운영 파라미터. ★슬라이스 A(코어 마이그레이션 195dbe5): SOXL/SOXS 기초지수 사실오류 정정 — PHLX SOX → NYSE Semiconductor Index(Bloomberg ICESEMIT), 2021-08-25 PHLX→ICE 교체·2023-11-03 ICE→NYSE 명칭변경(Direxion 공지·NYSE Arca RB-21-075·ICE 공지·SEC 497 교차검증), 잔존 4곳 전수 정정. 전 엔터티 네임스페이스 안정 ID + Q16 공통 스키마 + sourceRegistry 분리 + legacyIdMap 20키 + fail-closed resolve(). 명령형 처방 4곳 관찰 서술형 이관("손익비를 사전에 고정한다"·"당일 매매 중단"·"단타 한정 도구로 본다"·"사이즈 축소 또는 관망"), 출처 미특정 수치 삭제. legacyView() 호환 projection으로 유일 소비자 data-hub.js 무수정 통과(desk.js는 CORE 선언만 하고 미사용 확인). 검증기 확장: 금지표현 열거형 블랙리스트+부정문맥 화이트리스트 선적용, parameters[].value는 sourceRefs 필수·operational 상수 false, 중복ID·깨진참조·PHLX잔존·cachebuster 잔존 차단, RFC 8785 JCS→SHA-256 재해시 대조, --selftest 음성 7종·회귀 2종. ★슬라이스 B: 코어 확장 123 엔터티(용어 103·한계 9·셋업 4블록 4) + 출처 20건 + chart-playbook/ 9섹션 페이지. 캔들은 외부 차트 라이브러리 없이 인라인 SVG 직접 렌더, 합성 OHLC는 시드 고정 LCG로 재생성 바이트 동일, 성공/실패 짝은 같은 시드로 컷오프 이전 봉이 완전 동일(사후확증 차단을 구조로 보장). ★Codex 사후검수 5라운드 → 최종 PASS(f4a5ad4): R1 FAIL[3,4,5,8,9,11,12,14] → R2 FAIL[8,9,11,14] → R3 FAIL[8,9,11] → R4 FAIL[8] → R5 PASS(미해소 0). ※박사 지시 8항은 R1에서 이미 전부 PASS — 지시 4(진입 시점) "회피 아님", 지시 7(이평선·캔들 색) "축소 없음" 명시 판정. Codex가 잡아낸 실제 결함: ①입력 검증 부재(음수 거래대금·음수 ATR·교차 호가·0 주문금액·존재하지 않는 날짜가 산출 완료로 통과, 0 거래량에서 NaN 생성) ②검증기 거짓 양성(read-only에서 생성기 EPERM 실패인데 변경 없는 같은 파일을 비교해 "바이트 동일" 출력) ③CRLF 정규화가 바이트 동일성 주장을 무력화 ④실행 순서 결함 2건(비유한값 방어가 상태판정 뒤 / 축 무효화가 축간산술 뒤 → D-COST가 걸러진 값으로 선계산) ⑤종목 중립 위반(term.hbm이 신규 페이지 용어집에서 실존 기업명 노출) ⑥cachebuster 누락(HTML만 검사해 engine.js의 rules.json?v=20260615 놓침) ⑦출처 원문 불일치 2건(삼성 POP PDF가 인용 내용 미포함, SEC 497 발행일) ⑧Claude 보고와 실제 파일 불일치(추가했다고 보고한 테스트가 파일에 없었음 — 패치 후 실행 확인 규율 위반). ★최종 게이트: verify-all 한 명령 재현(쓰기 없음·read-only 안전) — 스키마+음성 11종·회귀 4종 / 원시 Buffer 바이트 동일 16024B + CRLF 오판 방지 / 짝 사전구간·규칙·역할 8/8 / JCS→SHA-256 재해시 8/8 / 성공·실패 동수 4:4 / 산출상태·완전성레벨·입력검증 119/119 / 레거시 소비자 회귀 0 / 헤드리스 9섹션·27 SVG·자료연결실패 0·undefined·NaN 0·실존 종목명 0. 연구·교육용·투자자문 아님(추천·목표가·승률·자동주문 없음).' },
      { type:'완료', title:'브로드컴·오라클 실적 후 주가 + SOXL/SOXS 카테고리 신설 (2026-06-24)', desc:'박사 지시 — 주식란에 브로드컴(AVGO)·오라클(ORCL) 올해 실적 발표 후 본주 주가 + SOXL·SOXS를 당일·다음날·1주일 조사해 별도 카테고리로. Codex 협업 검수. 신규 자급 페이지 broadcom-oracle-earnings/. ★실측: AVGO Q2 FY26(6/3 수 장후 발표, 매출 $22.2B +48%·EPS $2.44 비트) → 호실적인데 AI 연간목표 미상향·Q3 AI 가이던스 $16B(<$17.2B 컨센)로 6/4 -12.59%($479.23→$418.91, 갭 -14.7%·장중저 -15.9%), 1주일후 6/10 $372.10(-22.4%), 6/18 $411.35. ORCL Q4 FY26(6/10 수 장후, 매출 $19.2B +21%·EPS $2.11 비트) → FY27 $400억 조달·FCF -$237억·차입 우려로 6/11 -8.53%($201.26→$184.10), 1주일후 6/17 $183.53, 6/18 $184.29. ★SOXL/SOXS(3배 일일): AVGO 약한 AI 가이던스가 6/5 반도체 섹터 급락(보도 기준 SOX -10.3%·2020년 이후 최대·$1T+ 증발) 방아쇠 → SOXL -30.5%·SOXS +31.5%, 이후 6/11 +24%·6/16 -17%·6/18 +19% 롤러코스터. ORCL 반응일(6/11) 반도체는 반등 국면이라 SOXL +24% 디커플링. 레버리지 decay 경고(6/3~18 SOXL≈-0.4% vs SOXS -26.9%). ★데이터=Yahoo Finance(원시 차트 API)+stockanalysis.com 교차검증(종가·등락률 일치)+내부 등락률 검산 통과. Codex R1(데이터 정합·plausibility·설계) PASS+수정요구 반영(1주일 정의 정확화·SOXS decay 분리·레버리지 "일일·누적 3배 아님" 경고·SOX×3 근사 표기·외부 보도주장 vs 내부검산 분리). 허브 신규 섹션 카드 + 색상 상승빨강/하락파랑. 연구·교육용·투자자문 아님. 데이터셋 internal/notes/2026-06-24-avgo-orcl-earnings-dataset.json.' },
      ...
      { type:'핵심', title:'US-KR 카테고리 허브 신설 + Polaris Market Intelligence 개명 + Market Ledger 3번째 카테고리 (2026-06-11~13)', desc:'commit 0329843(06-11): US-KR 카테고리 허브 인덱스(index.html) 최초 신설 — 섹션 1(실시간·기록 3종: SOXL 실시간·Premarket Signal·Market Ledger) 구성, projects-data preview 초기 갱신. commit e4752e0(06-11): LIVE FEED CONNECTED 펄스 배지 추가. commit 1ff3874(06-13): 프로젝트명 공식 개명(「US-KR Premarket #6」 → 「Polaris Market Intelligence」) + Market Ledger 3번째 카테고리 신설(삼성전자·SK하이닉스·SOXL·SOXS 6월 일별 시세(본장/야간) + 미국·한국 6월 증시 캘린더, 실측·이중 교차검증). 랜딩 허브 2섹션으로 확장.' },
    ...
    milestones: [
      { isCore:true, date:'2026-09-01', title:'KR SYS 판독 매뉴얼 v2 교체 — 화면 전 요소 판독표로 확장', desc:'박사가 v2 원문을 내려주어 ⑫ 페이지를 원문 무수정 교체. 문서 성격이 바뀐 것이 핵심이다 — v1이 선을 읽고 피보나치를 직접 그리는 작도 매뉴얼이었다면, v2는 화면에 이미 그려져 있는 것(거래량 막대 4색, WPR, 다이버전스 4종, 우측 상단 13줄·좌측 하단 6줄 표)을 전수 판독하는 표에 가깝다. 피보나치도 수기 드래그에서 피벗 자동 탐지로 넘어갔고, 그 결과 v1의 ADI 워크드 예시와 작도 6단계는 삭제됐다. 새로 들어온 실전 판독 순서 8단계는 붉은 배경 → 충족 n/6 → 저점 확정 여부 순으로 1~2번에서 90%를 거른다고 스스로 명시한다. 세 종목 비교 예시의 기준일이 2026-09-02로 교체 시점보다 하루 뒤라는 점만 박사 확인 대기로 남겼다.' },
      { isCore:true, date:'2026-08-31', title:'KR SYS 지표 판독 매뉴얼 편입 — 육안 판정편 + 실측 대조', desc:'박사 지시로 자작 TradingView 지표 KR SYS 1·2의 판독 매뉴얼을 대시보드 ⑫로 편입. ⑪ 스크리너가 조건식을 코드로 전수 검증하는 쪽이라면, 이 문서는 같은 6조건을 차트 화면에서 눈으로 판정하고 골든크로스 자리에 피보나치를 대는 쪽이라 둘이 짝을 이룬다. 원문 무수정 편입 원칙을 지키되 검증 가능한 수치는 전부 실측 대조 — ADI 2025-11-24 ENTRY 종가 239.40과 되돌림 55.8%는 일치, 고점·저점 앵커 3건은 실제 봉과 불일치, 그리고 문서의 작도 절차를 실제 봉에 문자 그대로 적용하면 되돌림 85.4%로 진입 불가 판정이 나와 워크드 예시와 어긋난다는 점을 보고. ADI가 10/10·11/07·11/20 삼중 저점 박스권이라 파동 선택이 결과를 가르기 때문. 정정 여부는 박사 판단 대기.' },
      { isCore:true, date:'2026-08-31', title:'눌림목 반등 스크리너 — 6조건 조건식 303종목 전수 검증', desc:'박사가 조건식과 절대규칙(날조 금지·전 종목 순회·0개도 기록·실패 사례 포함·출처 명시)을 직접 지정하고 유니버스를 나스닥100+코스피200 둘 다로 선택. 303종목 × 2020-01-01~2026-08-31을 16배치로 순회해 신호 352건을 산출하고, 신호가 0개인 109종목과 판정불가 4종목까지 전부 기록. 시세는 yfinance(미국)·pykrx(한국) 이중 소스로 받아 삼성전자 종가 일치로 교차검증했고, AAPL 신호 1건은 6조건을 원자료 숫자로 수동 대조해 엔진을 검증한 뒤 배치를 돌렸다. 실행 중 pykrx가 거래정지 구간을 OHLC=0 가짜 봉으로 반환하는 결함을 적발해 27종목 278행을 제거하고 전량 재계산(SKT 최대낙폭 −100% → −7.41% 정정). 결과는 20일 승률 51.7%·60일 승률 56.8%·−8% 손절 터치 59.5%이며, 평균과 중앙값이 3.5배 벌어져 소수 대박이 평균을 끌어올린 구조임을 함께 표기. 생존편향은 시점별 편입이력 확보 불가로 보정하지 못해 박사 승인 하에 감수하고 전 표에 경고를 고정했다. 연구·교육용·투자자문 아님.' },
      { isCore:true, date:'2026-08-13', title:'SKHY 관측 대시보드 신설 — 스펙 검증 후 축소 구현', desc:'박사가 별도 스펙 문서(웹 Claude 작성, SKHY ADS 전환 추적)를 주고 구현 지시. 스펙 절대규칙 6에 따라 Phase 1 데이터 검증에서 멈추고 보고 → 박사 지시로 Codex(gpt-5.6-sol) 적대검토 → `PHASE1 DISPUTED` 판정 → 정정 후 Codex가 허용한 범위(관측 대시보드)로만 구현. ★Claude Phase 1의 오류 7건이 Codex 검토로 정정됨: 발행총수 개념 혼동(728,865,500은 outstanding, issued는 730,492,365) / 지분율 분모를 고정 상수로 잡으려 한 것(정답은 날짜별 상장주식수, 7/29 신주 상장일에 전환) / pykrx OHLCV를 KRX 함수로 오인(실제 Naver 경로) / 프리미엄 FX provenance 불일치 / FX 소스 비교의 as-of 불일치 / 거래대금을 실측으로 표기(실제는 종가×거래량 proxy) / 큰 괴리를 「경로 A 정황」으로 해석(원인 식별 불가). ★Codex가 새로 적발: 스펙의 ADS 수량 10배 오류(177,900,000 = 1억7,790만인데 스펙은 17억7,900만) / BASELINE_DATE 2026-07-14 이중계상 위험(IPO 기초주식이 누적 Gap에 유입) / KRX OpenAPI에 핵심 필드 부재로 계정 발급만으로 해결 불가 / F-6 archive registrant CIK가 issuer CIK와 상이 / 시그널 점수화 결함 12종으로 게시 불가. ★핵심 지표(외국인 보유주식수·순매수)는 KRX 인증 벽으로 확보 실패 — 추정·대체·보간 없이 「판정 불가」를 전면 표시. 정직의 한계선을 판별 대시보드가 아닌 관측 대시보드로 고정. ★산출: collect.mjs(receipt 보존)·verify.mjs(금지표현 부정문맥 밖 사용 차단 + 음성테스트)·정적 HTML 인라인 SVG. 원본 응답은 .gitignore로 공개 저장소 커밋 차단.' },
      { isCore:true, date:'2026-08-13', title:'차트 교본 신설 + Polaris Core v2 — 변증법 5라운드 후 구현', desc:'박사 지시로 단타·스윙 차트 교본을 신설. Claude(Opus 5) 제안 / Codex(gpt-5.6-sol EXEC read-only xhigh) 적대 검토 5라운드 — R1 DESIGN BLOCK(T1~T8) → R2 BLOCK(+체크리스트 20항) → R3 BLOCK(5·13·15·16·20) → R4 BLOCK(13·15·20) → R5 IMPLEMENTATION GO. 진행 중 박사 전제 정정("SOXL로 체결하지 않는다·종목 발굴이 목적·밸류에이션은 내가 찾는다")으로 산출물을 종목 중립 프레임으로 전환하고 종목 적합성 축을 신설, 오염된 R2를 중단·재발행. 합의 결론 — 스윙 = 2~20거래일(숏 2~5·코어 6~10·롱 11~20), 단타 = 스캘핑+인트라데이(정규장 종가 미통과), 「가장 중요한 지표」는 단일 지표가 아니라 계층이며 원자료(가격 OHLC+거래량)가 최상위·덧붙인다면 ATR(사용 봉 병기, 고정 손절폭 착시 방지 목적). 슬라이스 A = 코어 마이그레이션(기초지수 PHLX→NYSE Semiconductor Index/ICESEMIT 사실오류 정정, 전 엔터티 안정 ID·Q16 스키마·sourceRegistry, 명령형 처방 4곳 관찰형 이관, legacyView 호환으로 기존 화면 무수정 통과, 검증기 확장+음성 7종). 슬라이스 B = 엔터티 123(용어 103·한계 9·셋업 4)·출처 20·9섹션 페이지(인라인 SVG 캔들, 시드 고정 결정론 합성, JCS→SHA-256 사전조건 해시 대조, 성공/실패 짝의 컷오프 이전 봉 동일). 게이트 전부 PASS(스키마·음성/회귀·경로 30/30·레거시 회귀 0·헤드리스 렌더 0 오류). commit 195dbe5 · c3722fa.' },
      ...
      { isCore:true, date:'2026-06-11', title:'US-KR 카테고리 허브 신설 (Claude)', desc:'commit 0329843. 카테고리 허브 인덱스 최초 신설 — 섹션 1(실시간·기록 3종) 구성. e4752e0: LIVE FEED CONNECTED 펄스 배지.' },
      { isCore:true, date:'2026-06-13', title:'Polaris Market Intelligence 개명 + Market Ledger 신설 + AI 이중 예측 (Claude)', desc:'1ff3874: 공식 개명 + Market Ledger 3번째 카테고리. 29dfcd3·2783a0d·ecf6bcb: Opus 4.8·GPT 5.5 이중 예측 레이어·구성비중·ET/KST. b68d26c·550a6ee·3951413·3660167: 야간세션·전환·4세션 정합화.' },
  ...
  },
  {
    ...
    no: 7,
    name: 'Knowledgeverse',
    subtitle: '여러 분야의 지식을 하나의 「유니버스」로 묶는 상위 아카이브 — 시작페이지(카테고리 선택)에서 ① 기초과학(학교 과학 + 전기·전자 공학) ② 산업(반도체·전력·2차전지·디스플레이·철강·제련 3D) ③ 피트니스 정보(FitRx) ④ 추후 추가될 정보 로 진입 · A Curated Universe of Interactive Knowledge',
    icon: 'book',
    ...
    issues: [
      { type:'핵심', title:'기초과학 — 「전기·전자 공학」 카테고리 신설: 기초→중급→고급 3티어 (2026-07-09)', desc:'박사 지시 — 기초이론(기초과학)에 학교 과학과 별개의 「전기·전자 공학」 공학 과목을 새 TheorySubject로 신설. 난이도 3티어(기초=회로 리터러시 / 중급=반도체와 회로 / 고급=집적·신호·임베디드)를 SchoolLevel로 매핑, 총 14단원·42레슨·도해 14종(1차 11 + 이해-완결 보강 3). 기초=①부품·회로도 읽기 ②직류회로 설계·측정(멀티미터) ③커패시터·RC·신호 / 중급=①다이오드 ②트랜지스터(스위치·증폭) ③정류·전원회로 ④논리게이트 / 고급=①연산증폭기 ②집적회로·순차논리 ③ADC·DAC·신호처리 ④마이크로컨트롤러·임베디드(캡스톤). Opus 4.8 메인 설계 + Codex(gpt-5.5, EXEC read-only) 변증법 협업: R1 설계→Codex BLOCK(6항목: 반도체 도핑 중복 제거·RC/전원 경계 명문화·lint 스코프 한정·과목 스위처 상태계약·domainMeta 방어·수동 스모크 명시)→R2 IMPLEMENTATION GO→구현→Codex 사후검수 BLOCK(다이오드 순방향 전압강하 문장 절단)→정정→PASS. 학교물리 전자기(옴·쿨롱·전기장·유도·변압기)를 재유도하지 않고 반도체 소자·아날로그/디지털·연산증폭·신호처리·임베디드 공학으로 차별화(비중복 3렌즈 검증). 아키텍처: 미사용 THEORY_SUBJECTS export 활성화 + TheoryView 과목 스위처(세그먼트 pill) 신설, ScienceDomain에 "전기·전자 공학"(틸 #14b8a6·🔌) 추가 + domainMeta() 방어 fallback으로 무방어 크래시 4지점 제거, 과목 전환 시 level/grade/unit 원자 리셋. 콘텐츠 저작·적대검증·수정 24에이전트 워크플로 + 규율 lint(body 정확히 3문단·평서체·유니코드 수식·keyTerms 3~4·funFact) FAIL 0, 도해 SVG 11종(480×320 템플릿·resvg 렌더 검증). tsc/eslint/build PASS. newton fe9a8d6, 미리보기 재배포. ⚠️ semiconductor-universe main 미병합 — 박사 통합 디렉팅 대기. +이해-완결성 감사(5렌즈)로 essential 개념 공백 적발 → 박사 지시로 3단원 보강: 임피던스·리액턴스(기초④, 교류의 변하는 저항·위상·정합), 발진기(고급, 신호는 어디서 생기나·양성 되먹임·555), CPU 명령실행/메모리/버스(고급, 저장 프로그램이 도는 원리·MCU 앞) + 도해 3. 이해 사다리 바닥~꼭대기 완결(Codex R3 설계 GO·사후검수 PASS). newton 6e54c37.' },
      { type:'핵심', title:'Paralex 어휘 150카드 예문 고도화 + 문법 포인트(gp) 신설 + 예문 글자 확대 (2026-07-05)', desc:'디렉터 지시 2건. ① 예문 글자 확대 — Vocab Deck 카드의 영어 예문 11.5→14px·한국어 해석 11→13px(색 대비 상향), 카드 그리드 최소폭 255→310px. ② 예문 고도화+문법 병행 학습 — Day1~5 150카드 예문을 전부 14~24단어 복문·토익 비즈니스 문맥으로 재작성하고 카드마다 gp(문법 포인트) 신설: form=주절 기준 문장 형식(1~5형식·수동태·가주어 등) + points=용법(표제어 어법: assure+사람+that절 vs ensure+that절 등)·문법(예문 속 구조: 분사구문·관계사·도치)·출제(Part5/6가 속이는 방식: 품사 자리·유사어 바꿔치기·전치사 짝). 어휘 예문 하나로 Part 5·6 어법까지 병행 학습되는 구조(디렉터 아이디어). 생성→2렌즈 적대검증(영문법·형식 판정/토익 출제·번역 정합)→수정 워크플로 46에이전트, 10청크 전부 수렴·미해결 0. UI vg-gp 블록(형식 배지+용법/문법/출제 칩, 가림 blur 포함), validator.mjs gp 필수 게이트(용법·출제 필수) 추가 issueCount 0, AUTHORING.md 스키마 갱신. Edge 헤드리스: Day1 30카드×3포인트 전부 렌더 PASS.' },
    ...
    milestones: [
      { isCore:true, date:'2026-07-05', title:'Paralex 어휘 gp(문법 포인트) 시스템 — 150카드 전면 고도화', desc:'디렉터 지시 — 어휘 예문을 복문·토익 문맥으로 전면 재작성하고 카드마다 형식·용법·문법·출제 4요소 해설(gp)을 부착해, 어휘 카드가 Part 5·6 어법 학습을 겸하게 전환. 워크플로 46에이전트 생성·2렌즈 적대검증·수정 수렴(미해결 0), validator gp 게이트 추가(issueCount 0), 예문 폰트 확대(14px/13px)·그리드 확대. AUTHORING.md standardBatch 스키마에 gp 필수 반영.' },
  ...
  },
  {
    ...
    preview: { type:'embed', height:660, items:[
      { url:'https://gmpark-creator.github.io/project-dashboard/claude/previews/tradelogix-hub/', label:'VIEW → 카테고리 선택: 「통관절차」(부산항 통관 + Incoterms 2020 + 수입신고필증)와 「선사업무」(판다 익스프레스 라인 + MAGE 프로젝트 supply DB + 선박증서 31종)를 한 화면에서 선택·전환 — 보세사 뷰어는 1~5과목 2019~2025 전연도 875문항 + 2023년 1과목 B형(A·B형 토글) + 중요 개념·7개년 빈출(1~5과목 완성) + 정답 해설·빠른회독(1~5과목, 875문항 전수검증 PASS) + 희소 유형·낯선·저빈출(1~5과목, 40→33선 재컷) + 중빈도 유형·남은문항(1~5과목 41클러스터/100문항, ?mid=1)' }
    ]},
    ...
    issues: [
      ...
      { type:'완료', title:'보세사 1과목 2025년 PDF 원문 텍스트 대조 보정 (2026-06-12)', desc:'박사 지시 — 바탕화면 보세사 기출/기출과 정답/1과목 모음/25년 1과목.pdf를 렌더 이미지로 직접 대조해 bosesa-data-1-2025.js의 문제 본문·보기 텍스트를 공식 PDF 기준으로 보정. 이번 범위는 오탈자/문구 불일치 수정만이며 정답·오답·해설 검수는 후속 작업으로 분리. 상세 수정 항목은 internal/notes/2026-06-12-bosesa-subject1-2025-pdf-text-audit.md에 기록.' },
      { type:'완료', title:'보세사 1과목 2024년 PDF 원문 텍스트 대조 보정 (2026-06-12)', desc:'박사 지시 — 바탕화면 보세사 기출/기출과 정답/1과목 모음/24년 1과목.pdf를 렌더 이미지로 직접 대조해 bosesa-data-1-2024.js의 문제 본문·보기 텍스트를 공식 PDF 기준으로 보정. 이번 범위는 오탈자/문구 불일치 수정만이며 정답·오답·해설 검수는 후속 작업으로 분리. 상세 수정 항목은 internal/notes/2026-06-12-bosesa-subject1-2024-pdf-text-audit.md에 기록.' },
      { type:'완료', title:'보세사 1과목 2023년 PDF 원문 텍스트 대조 보정 (2026-06-12)', desc:'박사 지시 — 바탕화면 보세사 기출/기출과 정답/1과목 모음/23년 1과목.pdf를 렌더 이미지로 직접 대조해 bosesa-data-1-2023.js의 문제 본문·보기 텍스트를 공식 PDF 기준으로 보정. 이번 범위는 오탈자/문구 불일치 수정만이며 정답·오답·해설 검수는 후속 작업으로 분리. 상세 수정 항목은 internal/notes/2026-06-12-bosesa-subject1-2023-pdf-text-audit.md에 기록.' },
      { type:'완료', title:'보세사 1과목 2022년 PDF 원문 텍스트 대조 보정 (2026-06-12)', desc:'박사 지시 — 바탕화면 보세사 기출/기출과 정답/1과목 모음/22년 1과목.pdf를 렌더 이미지로 직접 대조해 bosesa-data-1-2022.js의 문제 본문·보기 텍스트를 공식 PDF 기준으로 보정. 이번 범위는 오탈자/문구 불일치 수정만이며 정답·오답·해설 검수는 후속 작업으로 분리. 상세 수정 항목은 internal/notes/2026-06-12-bosesa-subject1-2022-pdf-text-audit.md에 기록.' },
      ...
      { type:'완료', title:'보세사 희소 유형 · 낯선·저빈출 — 1~5과목 전과목 완성 (2026-06-22)', desc:'Codex 작업(369295c). 「중요 개념」의 반대편 — 2019~2025년 기출 중 반복 빈도가 낮은 절차형·예외형·기관/기간 매칭형·서식형 문제만 과목별 8선으로 선별한 「희소 유형 · 낯선·저빈출」 카테고리를 1~5과목 전과목 완성. 총 40선. 데이터: bosesa-rare-types.js (BOSESA_RARE_TYPES.subjects[s].items[]), 렌더러는 기존 bosesa-data 원본에서 실문제·정답·근거·정답선지해설을 참조(데이터 중복 없음), 과목마다 원문 출처 링크 버튼(fa-arrow-up-right-from-square). Claude 독립 4중 검수: ①node --check 42파일 ALL PASS ②인라인 스크립트 파싱 OK ③source-ref 감사(5과목 × 8항목, 필수 필드·watch≥3·원본 Q 존재·정답·근거 확인) issueCount=0 ④Chrome headless DOM 감사(cards/rareMeta/source/basis/analysis=8, wrong=0, sourceButtons=8) 5과목 PASS.' }
    ],
    milestones: [
      ...
      { date:'2026-06-12', title:'보세사 3·4·5과목 최종 패키지 통합 — 1~5과목 전연도 875문항 완성', desc:'GPT 웹 작업 최종 패키지(tradelogix_bosesa_FINAL_3_4_5_complete_2019_2025) 적용. 3과목 2019~2022, 4과목 2019~2025, 5과목 2019~2025 데이터를 누적 반영해 보세사 뷰어가 1~5과목 × 2019~2025 전연도 35세트(875문항)를 모두 연다. 2019년 과목명 예외(4과목=자율관리 및 관세벌칙, 5과목=수출입안전관리)와 복수정답 6건 유지. 패키지 검증기 PASS, Chrome headless로 18조합(3과목 2019~2022, 4·5과목 2019~2025) 전수 렌더/정답토글/active/복수정답 검증 PASS.' },
      { date:'2026-06-12', title:'보세사 1과목 2025년 — PDF 원문 문제·보기 텍스트 보정', desc:'바탕화면 보세사 기출/기출과 정답/1과목 모음/25년 1과목.pdf를 기준으로 2025년 1과목의 문제 본문과 보기 텍스트를 대조하고 오탈자·문구 불일치를 보정. 정답·해설은 변경하지 않고 후속 전수 검수 대상으로 남김. 상세 기록: internal/notes/2026-06-12-bosesa-subject1-2025-pdf-text-audit.md.' },
      { date:'2026-06-12', title:'보세사 1과목 2024년 — PDF 원문 문제·보기 텍스트 보정', desc:'바탕화면 보세사 기출/기출과 정답/1과목 모음/24년 1과목.pdf를 기준으로 2024년 1과목의 문제 본문과 보기 텍스트를 대조하고 오탈자·문구 불일치를 보정. 정답·해설은 변경하지 않고 후속 전수 검수 대상으로 남김. 상세 기록: internal/notes/2026-06-12-bosesa-subject1-2024-pdf-text-audit.md.' },
      { date:'2026-06-12', title:'보세사 1과목 2023년 — PDF 원문 문제·보기 텍스트 보정', desc:'바탕화면 보세사 기출/기출과 정답/1과목 모음/23년 1과목.pdf를 기준으로 2023년 1과목의 문제 본문과 보기 텍스트를 대조하고 오탈자·문구 불일치를 보정. 정답·해설은 변경하지 않고 후속 전수 검수 대상으로 남김. 상세 기록: internal/notes/2026-06-12-bosesa-subject1-2023-pdf-text-audit.md.' },
      { date:'2026-06-12', title:'보세사 1과목 2022년 — PDF 원문 문제·보기 텍스트 보정', desc:'바탕화면 보세사 기출/기출과 정답/1과목 모음/22년 1과목.pdf를 기준으로 2022년 1과목의 문제 본문과 보기 텍스트를 대조하고 오탈자·문구 불일치를 보정. 정답·해설은 변경하지 않고 후속 전수 검수 대상으로 남김. 상세 기록: internal/notes/2026-06-12-bosesa-subject1-2022-pdf-text-audit.md.' },
      { date:'2026-06-12', title:'보세사 1과목 2021년 — PDF 원문 문제·보기 텍스트 보정', desc:'바탕화면 보세사 기출/기출과 정답/1과목 모음/21년 1과목.pdf를 기준으로 2021년 1과목의 문제 본문과 보기 텍스트를 대조하고 오탈자·문구 불일치를 보정. 정답·해설은 변경하지 않고 후속 전수 검수 대상으로 남김. 상세 기록: internal/notes/2026-06-12-bosesa-subject1-2021-pdf-text-audit.md.' },
  ...
  },
  {
    id: 'creator-toolset',
    no: 13,
    name: 'Creator Toolset · Export Runner',
    subtitle: '유튜브용 영상 내보내기 + 이미지 추출 (무료 로컬 도구)',
    icon: 'film',
    platform: '로컬 데스크톱 · Python/FastAPI + ffmpeg',
    status: 'in-progress',
    start: '2026-06-28',
    latest: '2026-06-28',
    progress: 90,
    ...
    stackDetail: [
      { area: '영상 파이프라인', tech: 'ffmpeg (subprocess)', how: '정규화(CFR/스케일/색태깅)·concat·자막 번인·loudnorm 2-pass·libx264 인코딩을 명령으로 직접 제어. 결과물은 ffprobe로 규격(해상도·CFR·색·오디오·길이) 자동 검증.' },
      { area: '서버 / UX', tech: 'FastAPI + Vanilla JS', how: '비동기 잡(POST → 진행률 폴링 → 결과 패키지). 단일 HTML에 초보자용 1분 사용설명서 내장. 포트 8020, GPU 불필요(libx264 CPU).' },
      { area: '이미지 후처리', tech: 'Real-ESRGAN(ncnn-vulkan) + rembg', how: '씬 키프레임 + 균등 간격 후보 추출 후, 디렉터가 고른 1장만 업스케일/배경제거(휴먼인더루프). GPU/AI 엔진은 코어와 분리된 선택 모듈.' }
    ],
    issues: [
