...
const PROJECTS = [
  ...
  },
  {
    ...
    subtitle: '증권 — 미국·한국 반도체 단타(당일매매) 의사결정 지원 터미널 · 실시간·기록 3종(①SOXL 실시간 ②Premarket Signal ③6월 시세·이벤트) + 신규 트레이딩 도구·데이터 5종(④단타 데스크 ⑤시그널 엔진 ⑥0%선 전환 리플레이어 ⑦데이터·지식 베이스 ⑧단타 생존 가이드 5원칙) + 디렉터 코멘트(실전 트레이딩 로그) + 암호화폐 크로스 워치(BTC 골드/데드크로스) + ⑨차트 교본(종목 중립 — 단타·스윙 정의·캔들 도감·지표 계층·용어 100·구조적 마찰 관측기) — Claude·Codex 변증법 구축',
    icon: 'trending-up',
