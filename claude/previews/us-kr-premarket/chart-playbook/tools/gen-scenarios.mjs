/* chart-playbook/data/scenarios.js 생성기 (결정론·재현 가능)
 * 실행: node chart-playbook/tools/gen-scenarios.mjs
 *
 * 설계 요점 (Codex R4 체크리스트 15):
 *  - 짝(성공/실패)은 같은 LCG 시드로 「컷오프 이전 봉을 완전히 동일하게」 생성한다.
 *    → 동일 생성 규칙·동일 사전조건이 구조적으로 보장된다(문서 주장 아님).
...
const hash = o => crypto.createHash('sha256').update(jcs(o), 'utf8').digest('hex');
...
/* 종가 배열 → OHLC 봉 배열. 봉 내부 흔들림도 같은 LCG에서 뽑는다. */
...
/* ── 셋업별 경로 정의 ──
 * pre(): 컷오프 이전 종가 경로 (짝 공통)
 * post(): 컷오프 이후 종가 경로 (followThrough / failure 분기)
 */
const SHAPES = {
  'ma-pullback': {
    seed: 20260813,
    ...
    pre(rnd) {
      const out = []; let p = 100;
      for (let i = 0; i < 10; i++) { p += 1.1 + rnd() * 0.7; out.push(p); }        // 진행 구간
      for (let i = 0; i < 8; i++) { p -= 0.55 + rnd() * 0.35; out.push(p); }       // 되밀림(몸통 축소)
      return out;
    },
    post: {
  ...
  'range-breakout': {
    ...
    cutoff: 17,
    pre(rnd) {
      const out = []; const mid = 50;
      for (let i = 0; i < 18; i++) {
        const amp = 1.6 - i * 0.05;                                                // 범위 축소
        out.push(mid + Math.sin(i * 1.1) * amp + (rnd() - 0.5) * 0.35);
      }
      return out;
    },
    post: {
  ...
  'breakout-retest': {
    ...
    pre(rnd) {
      const out = []; let p = 30;
      for (let i = 0; i < 8; i++) { p += (rnd() - 0.5) * 0.5; out.push(p); }        // 구간
      for (let i = 0; i < 4; i++) { p += 1.0 + rnd() * 0.5; out.push(p); }          // 이탈
      for (let i = 0; i < 6; i++) { p -= 0.5 + rnd() * 0.3; out.push(p); }          // 경계 회귀
      return out;
    },
    post: {
  ...
  'opening-range': {
    ...
    cutoff: 11,
    pre(rnd) {
      const out = []; const base = 75;
      for (let i = 0; i < 12; i++) out.push(base + Math.sin(i * 1.5) * 0.9 + (rnd() - 0.5) * 0.4);  // 개장 구간
      return out;
    },
    post: {
...
const SPECS = {
  'pc.ma-pullback': {
    id: 'pc.ma-pullback', observationCutoff: 17, timeframe: '1d', session: 'regular',
    market: 'anonymous', instrumentType: 'equity', adjustment: 'adjusted',
    priorTrend: 'up', levelContext: '단기 이동평균 부근까지 되밀림', volumeContext: '되밀림 구간에서 거래량 감소',
    barsBeforeCutoff: 18, notes: '컷오프 이전 관측만 기술한다. 이후 봉·결과를 참조하지 않는다.',
  },
  'pc.range-breakout': {
    id: 'pc.range-breakout', observationCutoff: 17, timeframe: '1d', session: 'regular',
    market: 'anonymous', instrumentType: 'equity', adjustment: 'adjusted',
    priorTrend: 'sideways', levelContext: '고점·저점이 비슷한 자리에서 반복 정지, 범위 축소', volumeContext: '구간 내부 거래량 축소',
    barsBeforeCutoff: 18, notes: '컷오프 이전 관측만 기술한다. 이후 봉·결과를 참조하지 않는다.',
  },
  'pc.breakout-retest': {
    id: 'pc.breakout-retest', observationCutoff: 17, timeframe: '1d', session: 'regular',
    market: 'anonymous', instrumentType: 'equity', adjustment: 'adjusted',
    priorTrend: 'up', levelContext: '구간 상단 이탈 후 경계 부근으로 회귀', volumeContext: '회귀 구간 거래량이 이탈 봉보다 적음',
    barsBeforeCutoff: 18, notes: '컷오프 이전 관측만 기술한다. 이후 봉·결과를 참조하지 않는다.',
  },
  'pc.opening-range': {
    id: 'pc.opening-range', observationCutoff: 11, timeframe: '5m', session: 'regular',
    market: 'anonymous', instrumentType: 'equity', adjustment: 'adjusted',
    priorTrend: 'none', levelContext: '개장 직후 구간의 고가·저가 확정', volumeContext: '개장 구간에 거래량 집중',
    barsBeforeCutoff: 12, notes: '컷오프 이전 관측만 기술한다. 이후 봉·결과를 참조하지 않는다.',
  },
};
...
for (const [key, shape] of Object.entries(SHAPES)) {
  const m = META[key];
  const specHash = hash(SPECS[m.spec]);

  for (const role of ['followThrough', 'failure']) {
    // 짝마다 같은 시드에서 시작 → pre 구간 봉이 완전히 동일해진다.
    const rnd = lcg(shape.seed);
    const preCloses = shape.pre(rnd);
    const preBars = toBars(preCloses, rnd, shape.baseVol);
    // 분기: 이후 경로만 role에 따라 달라진다(별도 시드 오프셋 없이 같은 스트림 계속).
    ...
    scenarios.push({
      ...
      timezone: 'Asia/Seoul',
      adjustment: 'adjusted',
      generationRuleId: m.rule,
      preconditionSpecId: m.spec,
      preconditionHash: specHash,
      outcomeRole: role,
      pairedWithId: `scn.${key}.${role === 'followThrough' ? 'fail' : 'follow'}`,
      cutoffIndex: shape.cutoff,
      coreRefs: [m.core, 'limit.synthetic-not-evidence', 'limit.balanced-examples'],
      ohlc: preBars.concat(postBars),
    });
  }
}

/* 짝의 pre 구간이 실제로 바이트 동일한지 자체 확인 */
for (const key of Object.keys(SHAPES)) {
  const a = scenarios.find(s => s.id === `scn.${key}.follow`);
  const b = scenarios.find(s => s.id === `scn.${key}.fail`);
  const n = SHAPES[key].cutoff + 1;
  const same = JSON.stringify(a.ohlc.slice(0, n)) === JSON.stringify(b.ohlc.slice(0, n));
  if (!same) { console.error(`❌ ${key}: 짝의 컷오프 이전 봉이 다름`); process.exit(1); }
}

const fmtBar = b => `[${b[0]},${b[1]},${b[2]},${b[3]},${b[4]}]`;
const out = `/* Polaris Chart Playbook — 합성 시나리오 (window.POLARIS_SCENARIOS)
 *
...
console.log(`✅ scenarios.js 생성 — 시나리오 ${scenarios.length}건 (짝 ${scenarios.length / 2}쌍), 컷오프 이전 봉 동일성 확인 완료`);
