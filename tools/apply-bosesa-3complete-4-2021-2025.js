...
// Script tags. Public index currently has 3과목 2020~2025 only; this keeps all completion files available.
...
function yearButton(subject, year) {
  ...
  <i class="fa-solid fa-circle w-2 text-center text-violet-400"></i>${year}년
  </button>`;
}
...
function insertButtonAfter(subject, year, afterYear) {
  if (html.includes(`id="sb-s${subject}-year-${year}"`)) return true;
  const snippet = yearButton(subject, year);
...
if (!html.includes('id="sb-s4-menu"')) {
  ...
  if (locked4.test(html)) {
    ...
    if (idx >= 0) {
      ...
      warnings.push('Could not find 4과목 locked menu anchor. Apply PATCH_INDEX_3COMPLETE_4_2021_2025.diff manually.');
    }
  }
} else {
...
if (!html.includes('function toggleSubject4()')) {
  ...
  if(parent) parent.style.maxHeight = parent.scrollHeight + 400 + 'px';
...
const activeMarker = '/* BOSSA_PATCH_ACTIVE_YEARS_3COMPLETE_4_2021_2025 */';
if (!html.includes(activeMarker)) {
  const activeBlock = `
  ${activeMarker}
  [[3,[2022,2021,2020,2019]],[4,[2025,2024,2023,2022,2021]]].forEach(([subject, years])=>{
    years.forEach(year=>{
