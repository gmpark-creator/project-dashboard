...
export async function getLiveWorkerRetryPlan() {
...
export async function startLiveRender(projectId: string, input: LiveStartRenderInput) {
  if (!liveProjectWritesEnabled()) {
    throw new LivePersistenceUnavailableError("Live project writes are disabled. Set CUTPILOT_ENABLE_LIVE_WRITES=1 after running migrations.");
  }
...
export async function releaseLiveWorkerLease(leaseId: string, token: string | null | undefined) {
  if (!liveProjectWritesEnabled()) {
    throw new LivePersistenceUnavailableError("Live project writes are disabled. Set CUTPILOT_ENABLE_LIVE_WRITES=1 after running migrations.");
  }
  return withLivePersistenceClient((client) => new PostgresLivePersistenceWriteAdapter(client).releaseWorkerLease(leaseId, token));
}

export async function renewLiveWorkerLease(leaseId: string, input: { token?: string | null; ttlSec?: number } = {}) {
  if (!liveProjectWritesEnabled()) {
    throw new LivePersistenceUnavailableError("Live project writes are disabled. Set CUTPILOT_ENABLE_LIVE_WRITES=1 after running migrations.");
  }
...
export async function createLiveImageJob(input: LiveImageJobInput) {
  if (!liveProjectWritesEnabled()) {
    throw new LivePersistenceUnavailableError("Live project writes are disabled. Set CUTPILOT_ENABLE_LIVE_WRITES=1 after running migrations.");
  }
  return withLivePersistenceClient((client) => new PostgresLivePersistenceWriteAdapter(client).createImageJob(input));
}

export async function generateLiveShot(shotId: string, input: LiveShotGenerateInput = {}) {
  if (!liveProjectWritesEnabled()) {
    throw new LivePersistenceUnavailableError("Live project writes are disabled. Set CUTPILOT_ENABLE_LIVE_WRITES=1 after running migrations.");
  }
...
export async function upgradeLiveTake(takeId: string, input: LiveTakeUpgradeInput = {}) {
  ...
  return withLivePersistenceClient((client) => new PostgresLivePersistenceWriteAdapter(client).upgradeTake(takeId, input));
}
