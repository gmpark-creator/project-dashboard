...
function assertProductionWorkRequestBoundary() {
  ...
  const renderRouteSource = readFileSync(join(appApiDir, "projects", "[projectId]", "renders", "route.ts"), "utf8");
  ...
  assert.ok(renderRouteSource.includes("liveProjectWritesEnabled()"), "render route must require the live write switch for live work requests");
  assert.ok(renderRouteSource.includes("startLiveRender(projectId"), "render route must call the live render enqueue adapter");
  assert.ok(renderRouteSource.includes('apiError("LIVE_PERSISTENCE_UNAVAILABLE"'), "render route must fail closed when live persistence is unavailable");
  ...
  assert.ok(takeUpgradeRouteSource.includes("liveProjectWritesEnabled()"), "take upgrade route must require the live write switch for live work requests");
  ...
  assert.ok(liveRuntimeSource.includes("startLiveRender"), "live persistence runtime must expose live render enqueue");
  ...
  assert.ok(writeAdapterSource.includes("startRender"), "live write adapter must implement live render enqueue");
  ...
  assert.ok(writeAdapterSource.includes("INSERT INTO cutpilot_render_jobs"), "live write adapter must persist render jobs");
  assert.ok(writeAdapterSource.includes("buildLiveRenderPreview"), "live write adapter must use live render preview planning");
  assert.ok(writeAdapterSource.includes("upgrade_source_take_id"), "live write adapter must persist upgrade source takes");
  ...
  assert.ok(writeAdapterSource.includes('"upgradeTake"'), "live write adapter must reserve take upgrade credits");
  assert.ok(writeAdapterSource.includes('"startRender"'), "live write adapter must reserve render credits");
...
function assertProductionStateMutationBoundary() {
  ...
  const guardedRoutes = [
    ...
    {
      route: join(appApiDir, "system", "worker-leases", "[leaseId]", "complete", "route.ts"),
      path: "/system/worker-leases/{leaseId}/complete",
      method: "post" as const,
  ...
  assert.ok(selectTakeRouteSource.includes("liveProjectWritesEnabled()"), "select take route must require the live write switch for live state changes");
  assert.ok(selectTakeRouteSource.includes("selectLiveTake(shotId"), "select take route must call the live take selection adapter");
  assert.ok(selectTakeRouteSource.includes('apiError("LIVE_PERSISTENCE_UNAVAILABLE"'), "select take route must fail closed when live persistence is unavailable");
  ...
  const assetDeleteRouteSource = readFileSync(join(appApiDir, "projects", "[projectId]", "assets", "[assetId]", "route.ts"), "utf8");
  ...
  assert.ok(assetDeleteRouteSource.includes("liveProjectWritesEnabled()"), "asset delete route must require the live write switch for live state changes");
  assert.ok(assetDeleteRouteSource.includes("deleteLiveImageAsset(projectId"), "asset delete route must call the live asset delete adapter");
  assert.ok(assetDeleteRouteSource.includes('apiError("LIVE_PERSISTENCE_UNAVAILABLE"'), "asset delete route must fail closed when live persistence is unavailable");
  ...
  assert.ok(liveRuntimeSource.includes("deleteLiveImageAsset"), "live persistence runtime must expose live asset deletion");
  ...
  assert.ok(writeAdapterSource.includes("deleteImageAsset"), "live write adapter must implement asset deletion");
  ...
  assert.ok(writeAdapterSource.includes("DELETE FROM cutpilot_image_assets"), "live write adapter must delete image assets");
...
function assertProductionReadBoundary() {
  ...
  const workerRetryExecutionsRoute = readFileSync(join(appApiDir, "system", "worker-retries", "executions", "route.ts"), "utf8");
  ...
  assert.ok(queueSnapshotSource.includes("buildQueueSnapshotFromJobs"), "queue snapshot builder must support persisted job inputs");
  assert.ok(workerCompletionsSource.includes("buildWorkerCompletionSnapshotFromJobs"), "worker completion builder must support persisted job inputs");
  assert.ok(workerDispatchSource.includes("buildWorkerDispatchSnapshotFromJobs"), "worker dispatch builder must support persisted job inputs");
  ...
  assert.ok(workerRetryExecutionsRoute.includes("getLiveWorkerRetryExecutionSnapshot()"), "worker retry executions route must support live retry execution reads behind the switch");
  ...
  assert.ok(liveRuntimeSource.includes("getLiveWorkerRetryExecutionSnapshot"), "live persistence runtime must expose live worker retry execution snapshots");
  ...
  assert.ok(workerRetryExecutionsRoute.includes('apiError("LIVE_PERSISTENCE_UNAVAILABLE"'), "worker retry executions route must fail closed when live persistence is unavailable");
  ...
  assert.ok(workerRetryExecutionsRoute.includes('apiError("MOCK_READ_UNAVAILABLE"'), "worker retry executions route must fail closed in production without live reads");
