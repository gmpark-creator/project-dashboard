...
assert.equal(invalidProductionReadiness.ready, false, "production readiness should fail when required env has invalid shapes");
assert.ok(invalidProductionReadiness.invalidEnv.includes("CUTPILOT_QUEUE_URL"), "production readiness should report invalid queue URL env");
assert.ok(invalidProductionReadiness.invalidEnv.includes("CUTPILOT_ADMIN_TOKEN"), "production readiness should report invalid admin token env");
assert.ok(invalidProductionReadiness.checks.some((check) => check.id === "queue_worker" && check.status === "fail"), "invalid queue URL should fail the queue worker check");
process.env.CUTPILOT_QUEUE_URL = "https://queue.cutpilot.local/workers";
process.env.CUTPILOT_ADMIN_TOKEN = "valid-admin-token-123";
