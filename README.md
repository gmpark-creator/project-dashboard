...
- 미사용 기술스택 시각화 랩: https://gmpark-creator.github.io/project-dashboard/claude/stack-lab/
...
- **TradeLogix Nexus(#9) — 「미래 기술스택」을 3번째 카테고리로 통합, 별도 #10 폐지 (2026-06-06)**: 박사 지시 「산출물을 따로 빼지 말고 9번 프로젝트 안 3번 카테고리에서 보이게」 반영. 별도 프로젝트로 분리돼 있던 **#10 Tech Stack Collection을 폐지**하고, 그 산출물(기술 스택 아키텍처 & 진화 허브 + 미래 스택 실험 데모)을 **#9 TradeLogix Nexus의 세 번째 카테고리 「미래 기술스택」**으로 환원. `tradelogix-hub` 카테고리 선택 화면을 **통관절차/선사업무 2개 → 통관절차/선사업무/미래 기술스택 3개**로 확장(딥링크 `#tech` 추가, 3열 그리드), 아키텍처 허브 헤더에 실험 데모 링크 연결. 두 산출물 모두 헤더에 원래 "Project #9"로 자기표기돼 있어 정합. `projects-data.js`의 #10 블록·`index.html` 컬러맵에서 `tech-stack` 제거(대시보드 「프로젝트는 1개로 통합」 원칙). #9 progress 1% 유지, `node --check` PASS.
- **Knowledgeverse(놀리지버스, #8) — 브랜드명 확정 (2026-06-02)**: 프로젝트 #8 「정보·지식 모음」의 정식 브랜드를 **「Knowledgeverse / 놀리지버스」**로 확정(놀리지 아틀라스 → 놀리지 코스모스 → Knowledgeverse). 앱은 `src/brand.ts` 단일 소스로 워드마크·브레드크럼·타이틀에 반영, 대시보드 #8 카드·홈 '현재 판단' 메모도 신규 브랜드로 통일. ※ 아래 기존 항목들의 「반도체 유니버스(#8)」 표기는 당시 명칭 기준 기록(반도체 유니버스는 현재 영역1 이름).
- **반도체 유니버스(#8) — 8대 공정 모드 추가 (2026-06-01)**: 디렉터가 준 「반도체 8대 공정」 인포그래픽을 프로젝트의 **3D 스타일로 재구현**해 산업>반도체에 **세 번째 모드 「공정 과정」**을 추가. `ProcessFlow.tsx`가 8단계(웨이퍼 제조·산화·포토·식각·증착·금속배선·테스트·패키징)를 **뱀형(serpentine) 흐름 + 방향 화살표의 3D 웨이퍼 파이프라인**으로 그리고, 단계마다 **고유한 절차적 3D 비주얼**(실리콘 웨이퍼·투명 산화막·포토마스크 레티클·식각 트렌치·박막 적층 스택·금속배선 격자·프로브 니들·완성 IC 패키지)을 입힘. 단계 클릭 시 카메라 줌인 + 우측 패널에 **「왜 중요한가·특징·국내 기업·해외 기업」**. 데이터 주도(`AtlasArea.process` 선택 필드)라 전력·타 영역엔 무영향. `tsc+vite build`·ESLint 0·Playwright 헤드리스 검증(콘솔 에러 0). push `db120a3` + 미리보기 재배포.
- **반도체 유니버스(#8) — 2층 구조 + 기초이론 과학 트랙 통합·릴레이 동기화 (2026-06-01)**: 오늘 랩탑에서 진행한 **기초이론 과학 트랙**(앱을 산업↔기초이론 2층으로 확장, 초/중/고 전과정 + 진로·융합·심화 16과목 + 삼체 SF 과학 판별 + 도해 131종, Codex 구현·Claude 교차검수)을 `main`에 통합(`a06e3dd`)하고, 데스크탑에만 있던 **전력지도 딥줌** 2커밋도 `main` 합류(`3a3e7bb`). 미리보기를 전체 최신 빌드로 재배포. (#8 카드 stackDetail·이슈·마일스톤은 랩탑 릴레이가 별도 반영.)
- **2026 PRESIDENT KOREA(#7) — Codex 최신 진행 연동 + VIEW LIVE 현행화 (2026-05-31)**: Codex가 메인 개발을 인수(2026-05-29)해 구축한 게임플레이 엔진(월간 위기 생성·한국형 위기 템플릿, 세이브/로드, 월간 이벤트 덱, 국회 의석 협상, 부처·참모 능력치, 탄핵 리스크, 예산 회계연도, 게임 테스트 35/35)을 `korea-gov-sim` 레포에서 받아 빌드 → 대시보드 **VIEW LIVE 미리보기를 최신 빌드로 교체**(지도+국정현안+위기/이벤트/국회/예산이 도는 플레이 가능 vertical slice). #7 카드 stackDetail(게임 엔진)·이슈·마일스톤·요약 갱신, `latest 2026-05-29`, `progress 1%→35%`(현재 진행 반영). Codex 레포는 읽기만(빌드·반영) — push 없음. (Three.js 지도 NaN 콘솔 경고 1건은 Codex 영역 후속.)
- **반도체 유니버스(#8) — 화살표 연계 기업명 라벨 (2026-05-31)**: 공급망 화살표 각각에 **연계 기업명 라벨**(관계 색상 매칭)을 붙여, 연계 회사 핀이 화면 밖이어도 그 화살표가 **어느 기업과의 관계인지** 즉시 식별되게 함. 화살표 끝을 핀과 정확히 일치. push `5171afd`. (별개: 이 프로젝트 `.claude/settings.local.json`에 `defaultMode:bypassPermissions`를 추가해 **배쉬 권한 프롬프트 영구 차단** — 전역 bypass가 프로젝트 local 설정에 덮이던 문제 해결.)
